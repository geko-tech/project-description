...
public struct Template: Codable, Equatable {
    ...
    /// Attribute to be passed to `geko scaffold` for generating with `Template`
    @frozen
...
extension Template.Item {
    /// - Parameters:
    ///     - path: FilePath where to generate file
    ...
    /// - Parameters:
    ///     - path: FilePath where to generate file
    ///     - templatePath: FilePath of file where the template is defined
    /// - Returns: `Template.Item` that is `.file`
    public static func file(path: String, templatePath: FilePath) -> Template.Item {
        Template.Item(path: path, contents: .file(templatePath))
    }

    /// - Parameters:
    ///     - path: FilePath where will be copied the folder
    ///     - sourcePath: FilePath of folder which will be copied
    /// - Returns: `Template.Item` that is `.directory`
    public static func directory(path: String, sourcePath: FilePath) -> Template.Item {
